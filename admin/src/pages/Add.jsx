        ...
        <form onSubmit={handleSubmit} className='flex flex-col w-full items-start gap-3'>
            ...
            <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8 '>
                ...
                <div>
                    ...
                    <input value={price} onChange={(e) => setPrice(e.target.value)} className='w-full px-3 py-2 sm:w-[120px]' type="number" placeholder='25' min={1} />
                </div>
            </div>
                ...
                <div className='flex gap-3'>{
                    ["S", "M", "L", "Xl", "XXL"].map(
                        (el, index) => (
                            ...
                            <div className='' onClick={() => setSizes(prev => prev.includes(el) ? prev.filter(item => item !== el) : [...prev, el])} key={index}>
                                <p className={`${sizes.includes(el) ? " border" : ""} bg-slate-200 px-3 py-1 cursor-pointer `}>{el}</p>
                            </div>
                        )
                    )
                }
            ...
            <div className='flex gap-2 mt-2'>
                ...
                <input value={bestseller} onChange={() => setBestSeller(prev => !prev)} hidden type="checkbox" id="bestseller" />
                <label className={`${bestseller ? "bg-gray-400 text-white" : ""} border px-3 py-2  cursor-pointer`} htmlFor="bestseller">{!bestseller ? "Add to" : "Added to"} bestseller</label>
